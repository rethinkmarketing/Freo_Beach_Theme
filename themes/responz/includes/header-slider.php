...
if('on' === themify_get('setting-header_slider_enabled','on',true)) { ?>
	...
	<div id="header-slider" class="pagewidth slider">
		...
		$hasTitle=themify_get('setting-header_slider_hide_title',null,true);
		$autoplay = themify_get( 'setting-header_slider_auto', '0',true );
		$speed = themify_get( 'setting-header_slider_speed', 0.5,true )* 1000;
		$slider_args = apply_filters('themify_theme_header_slider_args',array(
			'data-auto'        => $autoplay!=='0'?$autoplay:false,
			'data-visible'      => themify_get( 'setting-header_slider_visible', 4,true ),
			'data-scroll'      => themify_get( 'setting-header_slider_scroll', 1,true ),
			'data-wrapvar'      => themify_get( 'setting-header_slider_wrap', true,true ),
			...
			'data-slider_nav'  => true,
                ...
                ?>
		<div data-lazy="1"<?php echo themify_get_element_attributes( $slider_args);?> class="slides tf_carousel tf_swiper-container tf_rel tf_overflow tf_clearfix">
			...
			if(themify_check('setting-header_slider_posts_category',true)){
				$cat = "&cat=".themify_get('setting-header_slider_posts_category','',true);
			} else {
				$cat = "";
			}
			if(themify_check('setting-header_slider_posts_slides',true)){
				$num_posts = "showposts=".themify_get('setting-header_slider_posts_slides','',true)."&";
			} else {
				$num_posts = "showposts=7&";
			}
			if( themify_get('setting-header_slider_display',null,true) === 'images'){
