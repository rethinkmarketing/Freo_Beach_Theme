...
class ThemifyBuilder_Data_Manager {
	...
	public static function save_data($builder_data, $post_id, $action = 'frontend',$custom_css=null) {
		...
		$result=self::update_builder_meta($post_id,$builder_data,false);
		unset($builder_data);
		if(!empty($result['mid'])){
	...
	public static function update_builder_meta($post_id,$data,$static_text=true,$transaction=true){
		...
		if($isRevision || $static_text===false || self::save_builder_text_only($post_id, $builder)){
			...
			if($isUpdate===true){
				...
				$wpdb->query( $wpdb->prepare( "DELETE FROM $wpdb->postmeta WHERE `post_id` = %d AND `meta_key` = %s", $post_id,self::META_KEY ));
			    }
			    //fires wp hooks
